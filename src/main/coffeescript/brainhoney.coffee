			...
			request = self.httplib.request(options, (response) ->
				...
				response.on('end', ->
					try
						...
						deferred.resolve(bilby.some(data))
		...
		withSession: (f) ->
			...
			new Http(@host, @port, @path).post(null, cmd: 'login', username: @username, password: @password)
				...
				.then((http) -> f(Object.freeze(
					...
					getEnrollmentMetricsReport: bilby.bind(http.get)(http, 'getenrollmentmetricsreport')
					listEnrollments: bilby.bind(http.get)(http, 'listenrollments')
					listEnrollmentsByTeacher: bilby.bind(http.get)(http, 'listenrollmentsbyteacher')
					listEntityEnrollments: bilby.bind(http.get)(http, 'listentityenrollments')
					listUserEnrollments: bilby.bind(http.get)(http, 'listuserenrollments')
					putSelfAssessment: bilby.bind(http.get)(http, 'putselfassessment')
					...
					# Resources.
					copyResource: bilby.bind(http.post)(http, 'copyresource')
					deleteDocuments: bilby.bind(http.post)(http, 'deletedocuments')
					deleteResources: bilby.bind(http.post)(http, 'deleteresources')
					getDocument: bilby.bind(http.get)(http, 'getdocument')
					getDocumentinfo: bilby.bind(http.post)(http, 'getdocumentinfo')
					getEntityResourceId: bilby.bind(http.get)(http, 'getentityresourceid')
					getResource: bilby.bind(http.get)(http, 'getresource')
					getResourceInfo2: bilby.bind(http.post)(http, 'getresourceinfo2')
					getResourceList2: bilby.bind(http.get)(http, 'getresourcelist2')
					listRestorableDocuments: bilby.bind(http.get)(http, 'listrestorabledocuments')
					listRestorableResources: bilby.bind(http.get)(http, 'listrestorableresources')
					putResource: bilby.bind(http.post)(http, 'putresource')
					putResourceFolders: bilby.bind(http.post)(http, 'putresourcefolders')
					restoreDocuments: bilby.bind(http.post)(http, 'restoredocuments')
					restoreResources: bilby.bind(http.post)(http, 'restoreresources')
					# Rights.
